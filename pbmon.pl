...
open (MYPROXIES, 'proxies.txt');
my @proxies = <MYPROXIES>;
chomp(@proxies);
$randomproxy = "http://$proxies[rand @proxies]/";
print "Using random proxy: $randomproxy\n";
...
open (MYFILE2, 'negatives.txt');
my @keywords = <MYFILE>;
my @negatives = <MYFILE2>;
chomp(@keywords) ;
my $regex = join('|', @keywords);
chomp(@negatives) ;
my $nregex = join('|', @negatives);
...
if (@links) {
	foreach $line (@links){
		...
		foreach $line (@data){
			...
			print $content;
			foreach $line (@data){
				if ($line =~ m/($regex)$/i && $line !~ m/($nregex)$/i){
				storeurl($link);
				print "$request $line\n";
				my $msg = MIME::Lite->new(
    					From    => 'PasteBinMon@argosproject.org',
    					To      => 'aamo@bdigital.org',
    					Cc      => '',
    					Subject => "Pastebin ($1) $request",
